...
export interface PDFReportData {
  ...
  insights: {
    ...
    taste_intersections?: any[];
    cross_domain_recommendations?: any[];
    created_at: string;
  };
}
...
export class PDFReportGenerator {
  ...
  private addTitle(title: string, fontSize: number = 16, color: [number, number, number] = [15, 23, 42]): void {
  ...
  private addMetricBox(label: string, value: string, color: [number, number, number] = [59, 130, 246]): void {
    const boxWidth = 40;
    const boxHeight = 20;
    ...
    this.pdf.roundedRect(this.margin, this.currentY - 5, boxWidth, boxHeight, 2, 2, 'F');
  ...
  private addProgressBar(label: string, percentage: number, color: [number, number, number] = [34, 197, 94]): void {
    this.addNewPageIfNeeded(15);
    ...
    // Percentage
    this.pdf.text(`${Math.round(percentage)}%`, this.margin + barWidth + 10, this.currentY);
  ...
  public async generateReport(data: PDFReportData): Promise<void> {
    ...
    if (data.project.cultural_domains?.length) {
      this.addText(`Cultural Domains: ${data.project.cultural_domains.join(', ')}`, 10);
    }
    if (data.project.geographical_targets?.length) {
      this.addText(`Geographical Targets: ${data.project.geographical_targets.join(', ')}`, 10);
    }
    
    this.currentY += 10;
    this.addText(`Report Generated: ${format(new Date(data.insights.created_at), 'MMMM d, yyyy \'at\' h:mm a')}`, 9);
    ...
    this.addMetricBox('Personas', data.insights.audience_personas?.length.toString() || '0', [59, 130, 246]);
    
    this.currentY = startY;
    this.pdf.text('', this.margin + 50, this.currentY); // Move to next column
    this.addMetricBox('Trends', data.insights.cultural_trends?.length.toString() || '0', [147, 51, 234]);
    
    this.currentY = startY;
    this.pdf.text('', this.margin + 100, this.currentY); // Move to next column
    this.addMetricBox('Content Ideas', data.insights.content_suggestions?.length.toString() || '0', [34, 197, 94]);
    ...
    if (data.insights.audience_personas?.length > 0) {
      ...
      data.insights.audience_personas.forEach((persona, index) => {
        ...
        this.addSubtitle(`${index + 1}. ${persona.name || `Persona ${index + 1}`}`);
        ...
        if (persona.demographics?.platforms?.length > 0) {
          this.addText(`Preferred Platforms: ${persona.demographics.platforms.join(', ')}`, 9);
        }
        ...
        if (persona.affinity_scores) {
          ...
          Object.entries(persona.affinity_scores).slice(0, 4).forEach(([domain, score]: [string, any]) => {
            this.addProgressBar(domain.replace('_', ' ').toUpperCase(), score * 100, [59, 130, 246]);
          });
        }
    ...
    if (data.insights.cultural_trends?.length > 0) {
      ...
      data.insights.cultural_trends.forEach((trend, index) => {
        ...
        this.addSubtitle(`${index + 1}. ${trend.title || `Trend ${index + 1}`}`);
        this.addText(trend.description || 'No description available');
        
        if (trend.confidence) {
          this.addProgressBar('Confidence Level', trend.confidence, [147, 51, 234]);
        }
    ...
    if (data.insights.content_suggestions?.length > 0) {
      ...
      data.insights.content_suggestions.forEach((suggestion, index) => {
        ...
        this.addSubtitle(`${index + 1}. ${suggestion.title || `Content Idea ${index + 1}`}`);
        this.addText(suggestion.description || 'No description available');
        
        if (suggestion.platforms?.length > 0) {
          this.addText(`Recommended Platforms: ${suggestion.platforms.join(', ')}`, 9);
        }
    ...
    if (data.insights.taste_intersections?.length > 0) {
      this.addTitle('Taste Intersections', 16, [34, 197, 94]);
      
      data.insights.taste_intersections.forEach((intersection, index) => {
        this.addNewPageIfNeeded(30);
        
        this.addSubtitle(`${index + 1}. ${intersection.intersection_name || `Intersection ${index + 1}`}`);
        this.addText(intersection.description || 'No description available');
        
        if (intersection.overlap_percentage) {
          this.addProgressBar('Overlap Percentage', intersection.overlap_percentage, [34, 197, 94]);
        }
        
        if (intersection.marketing_opportunities?.length > 0) {
          this.addText('Marketing Opportunities:', 9);
          intersection.marketing_opportunities.slice(0, 3).forEach((opportunity: string) => {
    ...
    if (data.insights.cross_domain_recommendations?.length > 0) {
      this.addTitle('Cross-Domain Recommendations', 16, [6, 182, 212]);
      
      data.insights.cross_domain_recommendations.forEach((recommendation, index) => {
        this.addNewPageIfNeeded(30);
        
        this.addSubtitle(`${index + 1}. ${recommendation.recommendation_title || `Recommendation ${index + 1}`}`);
        this.addText(recommendation.description || 'No description available');
        
        if (recommendation.source_domain && recommendation.target_domain) {
          this.addText(`Expansion: ${recommendation.source_domain} → ${recommendation.target_domain}`, 9);
        }
        
        if (recommendation.confidence_score) {
          this.addProgressBar('Confidence Score', recommendation.confidence_score, [6, 182, 212]);
        }
        
        if (recommendation.audience_fit) {
          this.addProgressBar('Audience Fit', recommendation.audience_fit * 100, [34, 197, 94]);
        }
        
        if (recommendation.expansion_opportunities?.length > 0) {
          this.addText('Expansion Opportunities:', 9);
          recommendation.expansion_opportunities.slice(0, 3).forEach((opportunity: string) => {
            this.addBulletPoint(opportunity);
          });
        }
...
export async function generatePDFReport(data: PDFReportData, filename?: string): Promise<void> {
  const generator = new PDFReportGenerator();
  await generator.generateReport(data);
  
  const defaultFilename = `${data.project.title.replace(/[^a-z0-9]/gi, '_').toLowerCase()}_insights_report.pdf`;
